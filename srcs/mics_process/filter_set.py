...
class FilterSet(object):
    ...
    class Type(Enum):
        ...
        PASSTHROUGH_FILTER = auto()
        """dummy filter used to return the ambisonic encoded signal, used to build an ambisonic encoder without changing the nature of the components built"""
    ...
    def create_instance_by_type(
        ...
        elif _type == FilterSet.Type.PASSTHROUGH_FILTER:
            return FilterSetPassthrough(
                file_name=file_name,
                is_hrir=False,
                sh_max_order=sh_max_order,
                sh_is_enforce_pinv=sh_is_enforce_pinv,
    ...
    def get_sh_configuration(self):
        ...
        with np.errstate(under="ignore"):
            if self._sh_is_enforce_pinv or self._irs_grid.weight is None:
                ...
                weights = self._irs_grid.weight
                weighted_bases = np.linalg.pinv(sh_bases)
