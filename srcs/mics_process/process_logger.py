...
_LOGGING_REPLACER = "@"
"""Character to replace in logging related configuration strings, see `system_config.LOGGING_FORMAT`. """
...
def setup(subprocess_name=None, is_disable_file_logger=False):
    ...
    if logger.hasHandlers():
        ...
        logger.setLevel(system_config.LOGGING_LEVEL)
        ...
        handler_std.setLevel(system_config.LOGGING_LEVEL)
...
def setup_logfile(name, ending="log"):
    ...
    `system_config`. Potentially already existing files from a former session get backed up with an
    ...
    try:
        file = os.path.abspath(system_config.LOGGING_PATH)
...
def _get_format_with_extending_name(current_length):
    """
    Get logging format string as set in `system_config`. In case it contains a placeholder for
    self-extending name length (see `system_config._LOGGING_SHOW_REPLACER`), it will be replaced by the
    ...
    """
    fmt = system_config.LOGGING_FORMAT
...
def _update_and_get_max_name_length(current_length):
    """
    Updates (or creates in case it was not existing) a parameter in `system_config`. It is storing the
    ...
    try:
        # noinspection PyUnresolvedReferences
        max_length = system_config.logging_show_name_length
    except AttributeError:
        max_length = 0  # attribute was not existing before

    if current_length > max_length:
        system_config.logging_show_name_length = current_length

    # noinspection PyUnresolvedReferences
    return system_config.logging_show_name_length
