...
import jack
import numpy as np
from . import tools, system_config, mp_context
...
class JackClient(SubProcess): 
    def __init__(
        ...
        def shutdown(status, reason):
            ...
            print('shutting down JACK status {status}, reason "{reason}"')
        ...
        def freewheel(starting):
            ...
            # print('{["stopping", "starting"][starting]} JACK freewheel mode.')

        # noinspection PyShadowingNames
        @self._client.set_blocksize_callback
        def blocksize(blocksize):
            if self._event_ready.is_set() and blocksize != self.client.blocksize:
                self._logger.error(
    ...
    def get_server_ports(self,is_audio = True, is_midi = False, is_input=False, is_output = False):

        return self._client.get_ports(is_audio=is_audio,is_midi=is_midi,is_input=is_input,is_output=is_output)
    ...
    def _client_register_and_connect_outputs(self, target_ports=True):
        ...
        if not len(self._client.outports):
            ...
            self._client_register_outputs(len(target_ports))
        
        # PAOLO: Commento causato dalla necessità di specificare la lista TODO: Sistemare in un modo migliore
