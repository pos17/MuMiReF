    ...
    def create_instance_by_filter_set(
        ...
        if not block_length:
            ...
            if filter_set.get_sh_configuration().arir_config:
                ...
                # for HRIR renderer
                if(system_config.IS_TEST_AMBISONIC):
                    convolver = AdjustableShConvolverAmbiRaw(
                        ## filter_set, block_length, source_positions, azim_deg=azim_deg,elevs_deg=elevs_deg ##shared_tracker_data
                        filter_set, block_length, source_positions, shared_tracker_data
                    )
                else:
        ...
        if system_config.IS_DEBUG_MODE and type(convolver) is not AdjustableShConvolverAmbiRaw:
...
class AdjustableShConvolverAmbiRaw(AdjustableShConvolver):
    """
    Extension of `AdjustableFdConvolver` to allow fast convolution in spherical harmonics domain
    while adjusting orientation by a rotation matrix i.e., for binaural rendering (depending on
    the head  orientation).
    ...
    def filter_block(self, input_block_td):
        ...
        # ):
        #     block_nm[0] += filter_block_nm * input_block_nm * ...
        input_block_nm_to_ret = input_block_nm.copy()
        # apply reverse index and adjust size according to filter channels
        input_block_nm = input_block_nm[self._sh_m_rev_id][:, np.newaxis, :]
        ...
        ##    output_out_block_td * self._window_out_td
        ##)
        output_block_to_ret_td = (output_in_block_td * self._window_in_td) + (
            output_out_block_td * self._window_out_td
        )   
        ## input_block_nm_to_ret = input_block_nm_to_ret[:,0,:]
        print("dims:")
        print(input_block_nm_to_ret.shape[0])
        print(input_block_nm_to_ret.shape[1])
        input_block_nm_to_ret_1 = input_block_nm_to_ret.reshape((1, input_block_nm_to_ret.shape[0], input_block_nm_to_ret.shape[1]))
        input_block_td_to_ret = self._ambi_raw_from_fd_to_td(input_block_nm_to_ret_1)
   
        to_ret = np.vstack((output_block_to_ret_td, input_block_td_to_ret))

        return to_ret

    def _ambi_raw_from_fd_to_td(self,input_nm):
        first_block_td = (
            self._irfft(input_nm)
            if system_config.IS_PYFFTW_MODE
            else np.fft.irfft(input_nm)
        )
        first_block_td = first_block_td[0, :, int(first_block_td.shape[-1] / 2) :]
        return first_block_td
    ...
    def get_output_channel_count(self):
        ...
        """
        # noinspection PyProtectedMember
        return self._filter._irs_td.shape[1] + self.calculate_sh_num()

    def calculate_sh_num(self):
        return (self._sh_cur_order+1)**2
